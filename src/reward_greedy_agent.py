    ...
    def action(self, task, mec_evn):
        ...
        if not found_task_done_high_priority:
            ...
            for i in range(len(Rewards)):
                max_reward_vector.append(max(Rewards[i]))
                max_reward_idx_vector.append(Rewards[i].index(max(Rewards[i])))
           
                                             
            max_reward=max(max_reward_vector)
